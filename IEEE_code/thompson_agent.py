...
def run_smart_ts(emab_episode: emab_model.AbstractEmabEpisode, num_weeks, user_arrivals, scale_factors=None):
    ...
    while week == 1 or len(user_set) > 0:
        ...
        for user in user_set:
            ...
            if feedback == -1 or user.emab_episode.t > DROPOUT_WEEK and feedback <= user.emab_episode.x / (
                    user.emab_episode.t - 1) and np.random.binomial(1, DROPOUT_PROB) == 1:
                if feedback != -1:
                ...
                ts_agent.update_rewards(best_action, feedback, user.emab_episode.cost_arr[-1],
                                        user.emab_episode.state_arr[-2] / 2, user.emab_episode.t - 1,
                                        user.emab_episode.state_arr[-2])
                max_feedback[user.emab_episode.t - 1] = max(feedback, max_feedback.get(0, user.emab_episode.t - 1))
    ...
    print('TS scale: {}'.format({i + 1: ts_agent_arr[i].scaling_factor for i in range(emab_episode.l_max - 1)}))
    return output
