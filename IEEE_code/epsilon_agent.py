...
def run_smart_epsilon(emab_episode: emab_model.AbstractEmabEpisode, num_weeks, user_arrivals):
    user_set = set()
    removal_set = set()
    week = 1
    num_users_added = 0
    ...
    while week == 1 or len(user_set) > 0:
        ...
        for user in user_set:
            ...
            if feedback == -1 or user.emab_episode.t > DROPOUT_WEEK and feedback <= user.emab_episode.x / (
                    user.emab_episode.t - 1) \
                    and np.random.binomial(1, DROPOUT_PROB) == 1:
                if feedback != -1:
                ...
                epsilon_agent.update_rewards(best_action, reward)
                max_feedback = max(max_feedback, reward)
